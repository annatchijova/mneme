...
| 4. `excluded_lineage` for partial exports | NOT done as named, and worth saying so plainly. B4's bilateral lineage check still skips when one party is absent, and R2-04 stands as written. What the same honesty contract now covers is broader: sweeps (B5), decisions (B8) and claim sets (B9) all declare what they cannot evidence rather than implying it. Lineage is the remaining gap, and it is listed in `KNOWN_LIMITATIONS.md` rather than quietly closed. |
